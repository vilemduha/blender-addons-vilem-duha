...
class panelData(bpy.types.PropertyGroup):
    ...
    #activated_category = bpy.props.BoolProperty(name="activated", default=False)
...
def drawTabsLayout(self, context, layout, tabpanel = None, operator_name = 'wm.activate_panel', texts = [], ids = [], tdata = [],  active = '', enable_hiding = False): #tdata=[],
...
def drawTabs(self,context,plist, tabID):
   ...
   # print('wau')
    ...
    if len(categories)>1: #EVIL TOOL PANELS!       
        ...
        if len(categories) > 1:
            if tabpanel_data.show:
                catops = drawTabsLayout(self, context, maincol,  tabpanel = tabpanel_data, operator_name = 'wm.activate_category', texts = sorted_categories, ids = sorted_categories, tdata = cdata,  active = active, enable_hiding = True)
    ...
    def execute(self, context):
        ...
        if not self.shift:
            for p in plist:
                ...
                #print((not hasattr( p, 'orig_category') or ( hasattr(item, 'orig_category') and p.orig_category == item.orig_category)), not hasattr( p, 'orig_category'), ( hasattr(item, 'orig_category') and p.orig_category == item.orig_category))
                if p.bl_region_type == panel.bl_region_type and p.bl_space_type == panel.bl_space_type and (not hasattr(panel,'bl_context') or p.bl_context==panel.bl_context) and (p.orig_category == apanel.orig_category):
...
class TabInterfacePreferences(bpy.types.AddonPreferences):
    ...
    disable_MODIFIERS = bpy.props.BoolProperty(name = 'Disable for modifiers and constraints', description = 'switch to/from hiding mode', default=False, update = updateDisabling)
