...
def blur(a):
    kernel = numpy.array([[1.0, 2.0, 1.0], [2.0, 4.0, 2.0], [1.0, 2.0, 1.0]])
...
def blur_iterations(a, iterations):
...
def restrictbuf(inbuf, outbuf):  # scale down array....
    ...
    if dx == 2 and dy == 2:  # much simpler method
        ...
        indices = numpy.arange(outx * outy * 2 * 3).reshape(
            (2, outx * outy, 3)
...
def solve_pde_multigrid(
    ...
    for a in range(0, levels + 1):
        ...
        VF.append(None)
        PLANAR.append(None)
    # float is deprecated, let's replace it
    VF[0] = numpy.zeros((xmax, ymax), dtype=float)
    # numpy.fill(pix)!? TODO
    ...
    for k in range(0, levels):
        ...
        sx = sx // 2
        sy = sy // 2
        PLANAR[k + 1] = numpy.zeros((sx, sy), dtype=float)
        RHS[k + 1] = numpy.zeros((sx, sy), dtype=float)
        IU[k + 1] = numpy.zeros((sx, sy), dtype=float)
        VF[k + 1] = numpy.zeros((sx, sy), dtype=float)
    ...
    # first let's calculate all the steps that have to be done:
    steps_total = levels * levels * vcycleiterations
    steps_done = 0
    ...
    for k in range(levels - 1, -1, -1):
        ...
        for cycle in range(0, vcycleiterations):
            ...
            for k2 in range(k, levels):
                ...
                steps_done += 1
                print(f"progress {steps_done/steps_total*100:.2f}%")
                if k2 != k:
                    IU[k2].fill(0.0)
...
def problemAreas(br):
    ...
    silh_thres = br.silhouette_threshold * 0.5
    ...
    silh_correction_x = numpy.abs(gx)
    for iter in range(0, br.silhouette_exponent):
        silh_correction_x = numpy.sqrt(silh_correction_x)
    silh_correction_y = numpy.abs(gy)
    for iter in range(0, br.silhouette_exponent):
        silh_correction_y = numpy.sqrt(silh_correction_y)

    # detect and also recover silhouettes:
    silhxpos = gx > silh_thres
    gx = (
        gx * (~silhxpos)
        + recover_silh * (silhxpos * silh_thres * silh_scale) * silh_correction_x
    )
    silhxneg = gx < -silh_thres
    gx = (
        gx * (~silhxneg)
        - recover_silh * (silhxneg * silh_thres * silh_scale) * silh_correction_x
    )
    ...
    gy = (
        gy * (~silhypos)
        + recover_silh * (silhypos * silh_thres * silh_scale) * silh_correction_y
    )
    silhyneg = gy < -silh_thres
    gy = (
        gy * (~silhyneg)
        - recover_silh * (silhyneg * silh_thres * silh_scale) * silh_correction_y
    )
...
def prepare_gradient(
    ...
    # separate function for gradient calculation
    # just to enable override of silhuette recovery and scale
    # scale down before:
    ...
    silh_correction_x = numpy.maximum(numpy.abs(gx) - silhouette_threshold, 0)
    # silhcorrectionx should
    silh_correction_x = (
        numpy.log(1.0 + (silh_correction_x)) * silhouette_threshold * silh_scale
        + silhouette_threshold
    )

    silh_correction_y = numpy.maximum(numpy.abs(gy) - silhouette_threshold, 0)
    silh_correction_y = (
        numpy.log(1.0 + (silh_correction_y)) * silhouette_threshold * silh_scale
        + silhouette_threshold
    )

    # detect and also recover silhouettes:
    silhxpos = gx > silhouette_threshold
    # we need to add the original gradient on non-silhouette pixels,
    # then add the recovered silhouette, but with a gradient of how much the silhouette is over the limit.
    # This should remove the artifacts where silhouette on the very edge of the limit becomes too noisy.
    gx = gx * (~silhxpos) + recover_silh * (silhxpos * silh_correction_x)
...
def relief(br):
    ...
    target_high_freequency = target.copy()
    ...
    if br.detail_enhancement_use:
        detail_divg, planar = prepare_gradient(
            br,
            recover_silh=True,  # do recover silhouettes?
            silh_scale=1.0,
            nar=nar,
            mask=mask,
            silhouette_threshold=br.silhouette_threshold * 0.5,
        )

        detail_divg, planar = prepare_gradient(br, False, 0.3, nar, mask)
        target_high_freequency = target.copy()
        solve_pde_multigrid(
            divg,
...
class BasReliefsettings(bpy.types.PropertyGroup):
    ...
    silhouette_scale: bpy.props.FloatProperty(
    ...
    min_gridsize: bpy.props.IntProperty(
        name="Minimum grid size", default=16, min=2, max=512
    )
    smooth_iterations: bpy.props.IntProperty(
        name="Smooth iterations", default=1, min=1, max=64
    )
    ...
    use_planar: bpy.props.BoolProperty(
        name="Use planar constraint", description="", default=False
    )
    ...
    # detail_enhancement_freq : bpy.props.FloatProperty(name="frequency limit", description="Image scale", min=0.025, max=1.0, default=.5, precision=PRECISION)
    detail_enhancement_amount: bpy.props.FloatProperty(
        name="amount",
...
class BASRELIEF_Panel(bpy.types.Panel):
    ...
    def poll(cls, context):
        ...
        return True  # rd.engine in cls.COMPAT_ENGINES
