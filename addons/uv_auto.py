...
bl_info = {
    ...
    "description": "Iteratively finds seams, and unwraps selected object. Generated nice quality islands. ",
    "warning": "",
    "wiki_url": "",
    "category": "UV",
}
...
def seedIslandsGrowth(context, bm, bm_dict, op):
    ...
    for f in bm.faces:
        # fweights[f.index] = vgroup_weights[f.verts[0].index]
        for v in f.verts:
            fweights[f.index] += (vgroup_weights[v.index]) / len(f.verts)  # / f.calc_area()
...
def continue_path_possibilities(v_path, weights, curvature_influence=0.5, threshold=1):
    ...
    nedges = []
    for i in edgesort:
        # this wasn't letting edges further;
...
class VPath():
    ...
    def copy(self):
        ...
        cp.lengths = self.lengths[:]
        cp.direction = self.direction.copy()
        cp.growing = True
        cp.hit_non_manifold = self.hit_non_manifold
        cp.met = self.met
        # cp.meets = []
...
def write_weights(weights, name, finish=False):
    # write to group here:
    ob = bpy.context.active_object
    ...
    normalized_weights = weights - minw
    ...
    for vi, w in enumerate(normalized_weights):
        vgroup.add([vi], w, 'REPLACE')
    #
...
def getCurvature(bm, smooth_steps=3, bm_dict=None):
    ...
    for ei in range(0, len(bm_dict.edges)):
        ...
        area = 0
        # for fi in bm_dict.e_faces[ei]:
        #     area += bm_dict.f_area[fi]
        for vi in bm_dict.e_verts[ei]:
            vgroup_weights[vi] += ea * bm_dict.e_lengths[ei]  # * area
    ...
    for a in range(0, smooth_steps):
        ...
        for v1i in range(0, len(bm.verts)):
            ...
            w = vgroup_weights[v1i] * (1)
            for v2i in neighbours:
                vgroup_weights_smooth[v2i] += w / len(bm_dict.v_neigbour_verts[v2i])
        vgroup_weights = vgroup_weights_smooth

    # write_weights(vgroup_weights,'curvature', finish = False)
...
def get_distant_verts(bm, vgroup_weights, seed_points = None):
    ''' gets most distant vertices from a starting point(by now index 0, should be bound box min/max points'''
    import bisect
    def get_second(l):
        return l[1]

    print('sort verts by topological distance')
    totverts = len(bm.verts)
    doneverts = {}
    distances = {}

    start_vert = bm.verts[0]
    # to_do = {start_vert.index: True}
    to_do = [(start_vert,0)]
    distances[start_vert.index] = 0

    done = False
    while len(to_do)>0:

        # print(to_do)
        to_do.sort(key = get_second)
        new_to_do = []
        for v1item in to_do:
            v1 = v1item[0]
            v1_dist = distances.get(v1.index)


            neighbour_dists = []
            for e in v1.link_edges:
                d = e.calc_length()
                neighbour_dists.append(d)
            neighbours_sorted = np.argsort(neighbour_dists)

            for ei in neighbours_sorted:
                e = v1.link_edges[ei]
                d = neighbour_dists[ei]
                v2 = e.other_vert(v1)
                v2_dist = distances.get(v2.index)
                v2_dist_trough_v1 = v1_dist + d
                # print(v2_dist_trough_v1)
                if v2_dist == None:
                    distances[v2.index] = v2_dist_trough_v1
                    # to_do[v2.index] = True
                    new_to_do.append((v2,v2_dist_trough_v1))
                # print(len(to_do))
                elif v2_dist > v2_dist_trough_v1:
                    #print(v2_dist, v2_dist_trough_v1)
                    distances[v2.index] = v2_dist_trough_v1
                    # to_do[v2.index] = True
                    new_to_do.append((v2,v2_dist_trough_v1))
        ...
        if len(to_do) == 0:
            ...
            print('restarting', len(doneverts))
            #probably for unconnected parts:
            for v in bm.verts:
                if distances.get(v.index) is None:
                    start_vert = v
                    # to_do = {start_vert.index: True}
                    to_do = [(start_vert, 0)]
                    distances[start_vert.index] = 0
                    break;
        # print(len(doneverts), totverts)
    # go through all distances. vertices which are local maxima/minima of the distance function get set as the top vertices.
    print('get top verts')
    topverts = []
    for v1 in bm.verts:
        v1_dist = distances[v1.index]
        all_smaller = True
        all_larger = True
        for e in v1.link_edges:
            v2 = e.other_vert(v1)
            v2_dist = distances[v2.index]
            if v2_dist > v1_dist:
                all_smaller = False
            elif v2_dist < v1_dist:
                all_larger = False
    ...
    # weights = np.zeros(totverts)
    # for v in bm.verts:
    #     weights[v.index] = distances[v.index]
    #
    # write_weights(weights, 'distance field', finish = True)
    return topverts
...
def seedIslandsCurvature(context, bm, bm_dict, op):
    ...
    # topverts = get_distant_verts(bm, vgroup_weights, seed_points=op.seed_points)
    ...
    for v in topverts:
        ...
        seed_edges = v.link_edges
        # seed_edges = v_counter_edges(v, vgroup_weights)
        for e in seed_edges:
            p1 = VPath()
            p1.index = pathindex
    ...
    while not done:
        ...
        i = 0
        grown = False
        growing_paths_count = 0
        for p1 in possible_paths:
            remove = []
            # print(p)
            # print(dir(p))
            if p1.growing:
                growing_paths_count += 1
                ...
                edges = continue_path_possibilities(p1, vgroup_weights, curvature_influence=op.curvature_influence,
                                                    threshold=1)
                npaths = [p1]
                path_branches_count = min(len(edges), 2)
                # we will branch out so get ready by copying paths first.
                for ei in range(1, path_branches_count):
                    p_old = p1
                    p1 = p_old.copy()
                    p1.siblings.append(p1)
                    p1.index = pathindex
                ...
                #     fal
                # now do the branches.
                for ei in range(0, path_branches_count):
                    p1 = npaths[ei]
                    v1 = p1.verts[-1]
                    ...
                    if not done_verts.get(v2.index):
                        ...
                        if tv1 == tv2:  # means paths from same source did meet, shorter wins.
                            # print('we met!')
                            v1i = len(p1.verts) - 1
                            v2i = p2.verts.index(v2)
                            # print(v1i,v2i,len(p1.verts), len(p2.verts), p1.lengths, p2.lengths)
                            if p1.lengths[v1i - 1] < p2.lengths[v2i - 1]:
                                done_verts[v2.index] = p1.index
                                p2.growing = False
                            else:
                                p1.growing = False
                            ...
                            p1.growing = False
                            # if len(p1.verts)<=p2.verts.index(v2):
                            p2.growing = False
                    ...
                    new_paths += 1
        print('number of growing paths: ', growing_paths_count)
        if not grown:
            done = True
...
def seed_with_merge(context, op):
    ...
    # TODO uncomment this !!! just to tune curvature.
...
class AutoSeamUnwrap(Operator):
    ...
    split_until_done: BoolProperty(
        name="Split until reaching good results.",
        description="splits all islands that don't fullfill conditions.",
    ...
    def draw(self, context):
        ...
        if self.show_advanced:
            ...
            layout.prop(self, 'split_until_done')
...
def menu_func(self, context):
    ...
    op.curvature_smooth_steps = 20
    op.seed_points = 30
    op.angle_deformation_ratio_threshold = 1.45
    op.area_deformation_ratio_threshold = 1.45
